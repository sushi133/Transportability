...
for (i in 1:12) {
  ...
  plot(surv,lty=1:1,col=c('black','red'),cex.axis=1,ymin=0.2,xlim=c(0,8))
...
colon0 <- subset(colon0, select = c(dyear5,dstat5,tx,age,gender,study,tx_age,tx_gender))
...
for (k in 1:12) {
  ...
  true <- numeric(12)
  target <- numeric(12)
  fit1<-coxph(Surv(dyear5,dstat5)~tx*age+tx*gender,data=colon0[colon0$study == levels(colon0$study)[k],])
  for (i in 1:12) {
    target[i]<-i
    ...
    fit2<-coxph(Surv(dyear5,dstat5)~tx*age+tx*gender,data=colon0[colon0$study == levels(colon0$study)[i],])
...
for (k in 1:12) {
  ...
  true <- numeric(12)
  target <- numeric(12)
  fit1<-aalen(Surv(dyear5,dstat5==1)~const(tx)+const(age)+const(gender)+const(tx_age)+const(tx_gender),data=colon0[colon0$study == levels(colon0$study)[k],],resample.iid=1)
  for (i in 1:12) {
    target[i]<-i
    c2<-colon0[colon0$study == levels(colon0$study)[i],]
    ...
    pred[i]<-c3[2,2]-c3[1,2]
    fit2<-aalen(Surv(dyear5,dstat5==1)~const(tx)+const(age)+const(gender)+const(tx_age)+const(tx_gender),data=colon0[colon0$study == levels(colon0$study)[i],],resample.iid=1)
...
for (k in 1:12) {
  ...
  true <- numeric(12)
  target <- numeric(12)
  fit1<-survreg(Surv(dyear5,dstat5)~tx*age+tx*gender,dist="lognormal",data=colon0,subset=(colon0$study == levels(colon0$study)[k]))
  for (i in 1:12) {
    target[i]<-i
    c2<-colon0[colon0$study == levels(colon0$study)[i],]
    ...
    fit2<-survreg(Surv(dyear5,dstat5)~tx*age+tx*gender,dist="lognormal",data=colon0,subset=(colon0$study == levels(colon0$study)[i]))
...
for (k in 1:12) {
  ...
  fit1<-coxph(Surv(dyear5,dstat5)~tx*age+tx*gender,weights=wt,data=p_colon0)
  c2<-colon0[colon0$study == levels(colon0$study)[k],]
  c2$hr<-as.numeric(unlist(summary(survfit(fit1,newdata=c2),times=5)$surv))
  c3<-aggregate(c2$hr, list(c2$tx), FUN=mean)
  pred<-c3[2,2]-c3[1,2]
  fit2<-coxph(Surv(dyear5,dstat5)~tx*age+tx*gender,data=colon0[colon0$study == levels(colon0$study)[k],])
  c2$hr<-as.numeric(unlist(summary(survfit(fit2,newdata=c2),times=5)$surv))
  c3<-aggregate(c2$hr, list(c2$tx), FUN=mean)
  true<-c3[2,2]-c3[1,2]
...
for (k in 1:12) {
  ...
  fit1<-aalen(Surv(dyear5,dstat5==1)~const(tx)+const(age)+const(gender)+const(tx_age)+const(tx_gender),weights=p_colon0$wt,data=p_colon0,resample.iid=1)
  c2<-colon0[colon0$study == levels(colon0$study)[k],]
  c2$hd<-predict(fit1,c2,times=5)$S0
  c3<-aggregate(c2$hd, list(c2$tx), FUN=mean)
  pred<-c3[2,2]-c3[1,2]
  fit2<-aalen(Surv(dyear5,dstat5==1)~const(tx)+const(age)+const(gender)+const(tx_age)+const(tx_gender),data=colon0[colon0$study == levels(colon0$study)[k],],resample.iid=1)
  c2$hd<-predict(fit2,c2,times=5)$S0
  c3<-aggregate(c2$hd, list(c2$tx), FUN=mean)
  true<-c3[2,2]-c3[1,2]
  tables <- rbind(tables, cbind(pred,true,-k,k))
...
for (k in 1:12) {
  ...
  fit1<-survreg(Surv(dyear5,dstat5)~tx*age+tx*gender,dist="lognormal",weights=wt,data=p_colon0)
  c2<-colon0[colon0$study == levels(colon0$study)[k],]
  pre<-survreg_calc_probs(c2,fit1,q=5,comparison = ">")$F
  c3<-aggregate(pre, list(c2$tx), FUN=mean)
  pred<-c3[2,2]-c3[1,2]
  
  fit2<-survreg(Surv(dyear5,dstat5)~tx*age+tx*gender,dist="lognormal",data=colon0,subset=(colon0$study == levels(colon0$study)[k]))
...
#plot
m1_s1$method<-'One-study-at-a-time'
m1_s1$survival<-'Proportional Hazards'
m1_s2$method<-'One-study-at-a-time'
m1_s2$survival<-'Additive Hazards'
m1_s3$method<-'One-study-at-a-time'
m1_s3$survival<-'Log-normal'
m2_s1$method<-'Leave-one-study-out'
m2_s1$survival<-'Proportional Hazards'
m2_s2$method<-'Leave-one-study-out'
m2_s2$survival<-'Additive Hazards'
m2_s3$method<-'Leave-one-study-out'
...
fdat$method <- factor(fdat$method, levels=c("One-study-at-a-time","Leave-one-study-out","Meta-analysis"))
...
ggplot(fdat, aes(x=study, y=mean, color=method, shape=method)) + 
  ...
  scale_y_continuous(breaks=seq(0,0.024,0.004),limits= c(0,0.024),expand = c(0.001,0.001),labels = comma) +
